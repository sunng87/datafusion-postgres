...
pub type Result<T> = std::result::Result<T, ToSqlError>;
...
impl ToSql for EncodedValue {
    fn to_sql(&self, _ty: &Type, out: &mut BytesMut) -> Result<postgres_types::IsNull>
    where
        Self: Sized,
    {
    ...
    fn to_sql_checked(&self, ty: &Type, out: &mut BytesMut) -> Result<postgres_types::IsNull> {
        self.to_sql(ty, out)
    }
}

impl ToSqlText for EncodedValue {
    fn to_sql_text(&self, _ty: &Type, out: &mut BytesMut) -> Result<postgres_types::IsNull>
    where
        Self: Sized,
    {
        out.writer().write_all(&self.bytes)?;
...
fn get_numeric_128_value(arr: &Arc<dyn Array>, idx: usize, scale: u32) -> Result<Option<Decimal>> {
    if arr.is_null(idx) {
        return Ok(None);
    }
    ...
    match arr.data_type() {
        ...
        DataType::Struct(_) => {
            let fields = match type_.kind() {
                ...
                _ => {
                    return Err(ToSqlError::from(format!(
